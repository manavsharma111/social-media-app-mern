...
try {
    ...
    const loop=await Loop.create({
        caption,media,audio,mediaType: mediaType || "video",author:req.userId
    })
...
export const deleteLoop = async (req, res) => {
    try {
        ...
        if(loop.author.toString() !== req.userId) return res.status(403).json({message: "Unauthorized"});
        
        await Loop.findByIdAndDelete(loopId);
        await User.findByIdAndUpdate(req.userId, { $pull: { loops: loopId } });
        
        io.emit("loopDeleted", { loopId });
